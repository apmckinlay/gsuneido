...
HorzFormat
	...
	layout(data, name, format)
		...
		if data.operation.Has?(#range)
			...
			items.Add([#Text, TranslateLanguage(text) $ ' '])
			items.Add(format)
			items.Add([#Text, ' ' $ TranslateLanguage("To") $ ' '])
			items.Add(format2)
			}
		else if data.operation.Has?("in list")
			items.Add([#Text,
				TranslateLanguage(data.operation) $ " (" $
					ParamsChooseListControl.
						DisplayValues(data.value,
							name).
						Join(", ") $ ')'])
		else if data.operation is #empty or data.operation is "not empty"
			items.Add([#Text, TranslateLanguage(data.operation)])
		else
			{
