...
class
	{
	CallClass(report, destinationFile, params = false, orientation = #Portrait,
		type = #pdf, maxPages = false)
		{
	...
	GenerateReportPDF(report, destinationFile, params, orientation, maxPages = false)
		...
		reportInstance = Report(@reportOb)
		try
			result = reportInstance.PrintPDF(destinationFile, quiet?:, :maxPages)
		catch (err)
			{
			reportInstance.Close(ReportStatus.ABORT)
			throw err
			}
		return result
		}
	...
	GenerateReportCSV(report, destinationFile, params, orientation)
		{
		reportOb = .buildReportObject(report, params, orientation, #csv)
		reportInstance = Report(@reportOb)
		try
			result = reportInstance.ExportCSV(destinationFile, quiet?:)
		catch (err)
			{
			reportInstance.Close(ReportStatus.ABORT)
			throw err
	...
	buildReportObject(report, params, orientation, reportType = #pdf)
		{
		reportOb = report.Prefix?(#Reporter)
			? .buildReporterObject(report, params)
			: .buildSystemReportObject(report, params)
		reportOb.paramsdata.ReportDestination = reportType
		// merging/including attachments is not supported by scheduled reports however
		// some reports assume that the EmailAttachments member is initialized to object
		if not reportOb.paramsdata.Member?(#EmailAttachments)
			reportOb.paramsdata.EmailAttachments = Object()
	...
	checkReportOptions(rpt, params)
		...
		nums =
			fields.Filter({ it.Has?("_abbrev") }).Map({ it.Replace("_abbrev", "_num") })
		fields.Append(nums)
