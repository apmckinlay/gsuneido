...
Document_Builder
	...
	AddArc(left, top, right, bottom, xStartArc/*unused*/, yStartArc/*unused*/,
		...
		.stream $= (left < right and bottom < top) or (left > right and bottom > top)
			? (left $ ' ' $ bottom $ " l\n")
			: (right $ ' ' $ top $ " l\n")
		.stream $= "h\nW\nn\n"
	...
	AddLine(x, y, x2, y2, thick, color = 0x0000_0000)
		{
		x = x / .pspToTwip
	...
	AddRect(x, y, w, h, thick, fillColor = false, lineColor = false)
		...
		.doWithColorAndBorder(thick, fillColor, lineColor)
			{
			.stream $=
				x $ ' ' $ y $ ' ' $ w / .pspToTwip $ ' ' $ (-h / .pspToTwip) $ " re h "
			}
		}
	...
	doWithColorAndBorder(thick, fillColor, lineColor, block)
		...
		if fillColor isnt false
			{
			b = ((fillColor & 0xFF_0000) / 0x1_0000 / 0xFF).Round(5)
			g = ((fillColor & 0x00_FF00) / 0x100 / 0xFF).Round(5)
			r = ((fillColor & 0x00_00FF) / 0xFF).Round(5)
			.stream $= r $ ' ' $ g $ ' ' $ b $ " rg\n"
			}
		if lineColor isnt false
			{
			b = ((lineColor & 0xFF_0000) / 0x1_0000 / 0xFF).Round(5)
			g = ((lineColor & 0x00_FF00) / 0x100 / 0xFF).Round(5)
			r = ((lineColor & 0x00_00FF) / 0xFF).Round(5)
			.stream $= r $ ' ' $ g $ ' ' $ b $ " RG\n"
			}
		if thick isnt 0
	...
	circle(radius, x, y)
		...
		.stream $= (x - radius) $ ' ' $ y $ " m\n"
		.stream $= (x - radius) $ ' ' $ (y + distance) $ ' ' $ (x - distance) $ ' ' $
			(y + radius) $ ' ' $ x $ ' ' $ (y + radius) $ " c\n"
		.stream $= (x + distance) $ ' ' $ (y + radius) $ ' ' $ (x + radius) $ ' ' $
			(y + distance) $ ' ' $ (x + radius) $ ' ' $ y $ " c\n"
		.stream $= (x + radius) $ ' ' $ (y - distance) $ ' ' $ (x + distance) $ ' ' $
			(y - radius) $ ' ' $ x $ ' ' $ (y - radius) $ " c\n"
		.stream $= (x - distance) $ ' ' $ (y - radius) $ ' ' $ (x - radius) $ ' ' $
			(y - distance) $ ' ' $ (x - radius) $ ' ' $ y $ " c\n"
	...
	roundRect(x1, y1, x2, y2, width, height)
		...
		.stream $= p1.x $ ' ' $ p1.y $ " m\n"
		.stream $=
			p2.x $ ' ' $ p2.y $ ' ' $ p3.x $ ' ' $ p3.y $ ' ' $ p4.x $ ' ' $ p4.y $ " c\n"
		.stream $= p5.x $ ' ' $ p5.y $ " l\n"
		.stream $=
			p6.x $ ' ' $ p6.y $ ' ' $ p7.x $ ' ' $ p7.y $ ' ' $ p8.x $ ' ' $ p8.y $ " c\n"
		.stream $= p9.x $ ' ' $ p9.y $ " l\n"
		.stream $= p10.x $ ' ' $ p10.y $ ' ' $ p11.x $ ' ' $ p11.y $ ' ' $ p12.x $ ' ' $
			p12.y $ " c\n"
		.stream $= p13.x $ ' ' $ p13.y $ " l\n"
		.stream $= p14.x $ ' ' $ p14.y $ ' ' $ p15.x $ ' ' $ p15.y $ ' ' $ p16.x $ ' ' $
			p16.y $ " c\n"
		.stream $= p1.x $ ' ' $ p1.y $ " l\nh\n"
		}
	...
	ellipse(w, h, x, y)
		...
		centreY = y + h/2
		.stream $= x $ ' ' $ centreY $ " m\n"
		.stream $= x $ ' ' $ (centreY + distanceY) $ ' ' $ (centreX - distanceX) $ ' ' $
			(y + h) $ ' ' $ centreX $ ' ' $ (y + h) $ " c\n"
		.stream $= (centreX + distanceX) $ ' ' $ (y + h) $ ' ' $ (x + w) $ ' ' $
			(centreY + distanceY) $ ' ' $ (x + w) $ ' ' $ centreY $ " c\n"
		.stream $= (x + w) $ ' ' $ (centreY - distanceY) $ ' ' $ (centreX + distanceX) $
			' ' $ y $ ' ' $ centreX $ ' ' $ y $ " c\n"
		.stream $= (centreX - distanceX) $ ' ' $ y $ ' ' $ x $ ' ' $
			(centreY - distanceY) $ ' ' $ x $ ' ' $ centreY $ " c\n"
		}
	...
	*/
	buildPage()
		...
		resources = "<</Type /Page /Parent 2 0 R /MediaBox [0 0 " $ .docWidth $ ' ' $
			.docHeight $ "] " $ "/Contents [" $ (.writer.NextObjId + 1) $
			" 0 R] /Resources<<\n" $ "/Font <<\n" $ fontReferences $ ">>\n" $ xObjRefs $
			">>>>\n" $ "endobj\n"
	...
	*/
	...
	GetCharWidth(width, font, widthChar)
		...
		if width is false
			return 0
		font = PdfFonts.GetCompatibleFont(font)
		return width * PdfFonts.GetCharWidth(font, widthChar) * font.size * .afmToTwip
		}
	...
	*/
	...
	Finish(status)
		...
		if status is ReportStatus.SUCCESS
			{
			.writer.AddObject(
				"<</Type /Pages /Kids [" $ .buildPageRefString() $ "] /Count " $ .pages $
					">>\n" $ "endobj\n", id: 2)
