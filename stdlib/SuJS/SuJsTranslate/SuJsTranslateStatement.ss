...
SuJsTranslate // to get output stuff
	...
	ExprStmt(ast, last)
		...
		if last
			.Print('return ')
		SuJsTranslateExpression(ast.expr, discard: not last)
		.Println(';')
		}
	...
	MultiAssign(ast)
		...
		.Print('] = (')
		SuJsTranslateExpression(ast[size - 1])
		.Println(');')
		}
		...
		if _inblock
			...
			if ast.size > 1 // multiple returns
				...
				for i in ..ast.size
					...
					if i > 0
						.Print(', ')
					SuJsTranslateExpression(ast[i], discard:)
					}
				.Print(']')
				}
	...
	While(ast)
		{
		.Print('while (')
		SuJsTranslateExpression.Bool(ast.cond)
		.Print(')')
		.loopBody(ast.body)
		}
	For(ast)
	...
	forRange(ast)
		...
		.Println(');')
		.Print(to $ ' = (')
		SuJsTranslateExpression(ast.expr2)
		.Println(');')
	...
	If(ast)
		...
		if ast.f is false or
			...
			if ast.f.type is 'If'
				{
				.Print(' ')
				SuJsTranslateStatement(ast.f) // recursive
				}
			else
				.bodyln(ast.f)
			}
					...
					for (k = 0; k < body.size; k++)
						...
						if body[k].type is 'Compound'
							for (p = 0; p < body[k].size; p++)
								SuJsTranslateStatement(body[k][p])
						else
							SuJsTranslateStatement(body[k])
						}
		...
		.Indented()
			...
			if catchPart.type is 'Compound'
				for (i = 0; i < catchPart.size; i++)
					SuJsTranslateStatement(catchPart[i])
			else
				SuJsTranslateStatement(catchPart)
			}
		.Println('}')
		}
	Throw(ast)
	...
	exprList(list) // list is an object of exprs
		...
		for (i = 0; i < list.Size(); ++i)
			...
			if i > 0
				.Print(', ')
			SuJsTranslateExpression(list[i], discard:)
			}
		}
	bodyln(ast)
		{
	...
	body(ast)
		...
		.Indented()
			...
			if ast.type is 'Compound'
				for (i = 0; i < ast.size; i++)
					SuJsTranslateStatement(ast[i])
			else
				SuJsTranslateStatement(ast)
			}
		.Print('}')
		}
	}
