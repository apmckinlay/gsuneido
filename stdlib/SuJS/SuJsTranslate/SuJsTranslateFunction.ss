...
SuJsTranslate // inherit to get output stuff
	{
	...
	handleDotParams(params)
		...
		for (i = 0; i < params.size; i++)
			...
			if _inMethod
				memberName = SuJsTranslateClass.PrivatizeName(memberName)
	...
	body(stmts)
		...
		for (i = 0; i <= last; ++i)
			...
			SuJsTranslateStatement(stmts[i], i is last)
			}
		// declare variables
		.At(pos)
			{
