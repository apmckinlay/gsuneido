...
class
	...
	CreatePreauthToken()
		{
		token = Base64.Encode(Random.Bytes(16/*=len*/))
		ServerSuneido.Add(#SuPreauthTokens, Date().Plus(minutes: 5), token)
		return token
		}
	...
	validatePreauthToken(env)
		...
		if false is expiry = ServerSuneido.GetAt(#SuPreauthTokens, token, false)
			return false
