...
Test
	...
	cl: SuJsLoadRuntime
		...
		New(book, .info)
			{
			.SuJsLoadRuntime_book = book
			}

		SuJsLoadRuntime_maxChunkSize: 10
		SuJsLoadRuntime_tags()
			{
			return .info.tags
			}

		SuJsLoadRuntime_tempName()
			{
			return "import" $ .info.n++
			}

		SuJsLoadRuntime_fileSize(name)
			{
			return .info.files[name].Size()
			}

		SuJsLoadRuntime_readFile(name, block)
			{
			FakeFile(.info.files[name], block)
			}

		SuJsLoadRuntime_putFile(name, s)
			{
			Assert(not .info.putFiles.Member?(name))
			.info.putFiles[name] = s
			}

		SuJsLoadRuntime_deleteFile(name)
			{
	...
	Test_prepareFiles()
		...
		info = Object(n: 0,
			...
			putFiles: Object())
		cl = new .cl(.book, info)
		fn = cl.SuJsLoadRuntime_prepareFiles

		Assert(fn("short.js") is: #([path: "short.js", temp?: false, partNum: ""]))
		Assert(fn("long.js")
			is: [[path: #import0, temp?:, partNum: 0],
	...
	Test_QueryRec()
		...
		Assert(result
			hasSubset: [text: "part one part two",
				lib_modified: #20250103, lib_committed: #20250104])
	...
	Test_importFiles_queryRec()
		...
		cl.SuJsLoadRuntime_importFiles("small.js", "small__prod.js")
		...
		cl.SuJsLoadRuntime_importFiles("large.js", "large__prod.js")
		...
		cl.SuJsLoadRuntime_importFiles("exact.js", "exact__prod.js")
		...
		// First import
		cl.SuJsLoadRuntime_importFiles("replace.js", "replace__prod.js")
		...
		cl.SuJsLoadRuntime_importFiles("tagged.js", "tagged__prod.js")
