...
Test
	...
	Test_one()
		...
		info.When.GetV2Support([anyArgs:]).CallThrough()
...
Caption : Intel64 Family 15 Model 55 Stepping 6
...
`
		...
		Assert(info.V2Support hasPrefix: 'No -')
...
Caption : BlackMarket64 Family zero RandomProcessorChip
...
`
		...
		Assert(info.V2Support hasPrefix: 'No - unable to parse processor caption')
...
Caption : BlackMarket64 Family 0 Model 0
...
`
		...
		Assert(info.V2Support hasPrefix: 'No - Unknown Chip:')
...
SMBIOSBIOSVersion : 1.0
ReleaseDate       : 10/15/2017 8:00:00 PM
Caption : AMD64 Family 21 Model 22 Stepping 10
`
		info = Mock(SystemInfo)
		info.When.GetV2Support([anyArgs:]).CallThrough()
		info.Eval(SystemInfo.SystemInfo_parseInfo, example)
