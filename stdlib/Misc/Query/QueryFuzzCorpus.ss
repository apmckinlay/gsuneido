...
#(
	...
	`(((((((cus union cus) extend r0)) where c3 is "")) extend i4 = r0) union cus)`,
	`(((cus union ((((cus union cus) extend r0)) extend ik = r0)) union cus) union cus)`,
	`((((ivc union ivc) leftjoin ((aln extend r0) union bln)) extend x1 = r0) union
		((ivc union ivc) join aln)) remove i1`,
	`((((ivc union ((ivc extend r0) union ivc)) join bln) extend x1 = r0) union
		(ivc join bln) remove i1,i4) sort b3`,
	`((ivc where ck is "") leftjoin (cus union ((((cus extend r0)) extend a4 = r0) union cus)))
		sort ck,ik,i2`,
	`(cus remove c1,c4 union cus) union ((cus extend x0 = "0") union cus)`,
	`(ivc leftjoin (cus remove c3,c1 union cus)) union (cus join ivc)`,
	`(((ivc union ivc) join bln) union (ivc join (bln where bk is "3")))`,
	`((cus extend ik = c3) join ivc) join (aln union (bln union aln))`,
	)
