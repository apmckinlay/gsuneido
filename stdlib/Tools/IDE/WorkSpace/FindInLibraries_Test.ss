...
Test
	...
	Test_linesExpr()
		...
		data.bycomby = 'return :[a]'
		Assert(f(data)
			like: 'Object(#RegexMatchLines, lib_current_text, "(?i)(?q)foo(?-q)"),
				Object(#FindByToken, lib_current_text, #("is", "foo"), #(`\<foo\>`)),
				Object(#FindByComby, name, lib_current_text, "return :[a]", false)')

		data.bycomby = 'return abc'
		Assert(f(data)
			like: 'Object(#RegexMatchLines, lib_current_text, "(?i)(?q)foo(?-q)"),
				Object(#FindByToken, lib_current_text, #("is", "foo"), #(`\<foo\>`)),
				Object(#FindByComby, name, lib_current_text, "return abc", "abc")')
