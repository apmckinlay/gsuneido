...
Controller
	...
	List_WantEditField(col)
		...
		return .colName(col) is #Comment
			? [#Editor, xmin: .list.GetColWidth(2), height: 15, readonly:]
			: false
		}
	...
	List_GetDlgCode(lParam)
		...
		if false isnt (m = MSG(lParam)) and
			(m.message is WM.CHAR or m.message is WM.KEYDOWN) and m.wParam is VK.RETURN
	...
	showdiff2(name1, when1, name2, when2, comment = "")
		...
		.showdiff("As of " $ when1.ShortDateTime() $ Opt(" - ", rec1.path), rec1.text,
	...
	view(when)
		...
		viewer = BookResource?(name, imagesOnly?:) ? ImageViewer : CodeViewer
	...
	On_Diff_to_Current()
		...
		.showdiff(x.When.ShortDateTime() $ " - " $ rec.path, rec.text,
	...
	locateFirstTimeExist(listData, line)
		...
		searchIncrement = 10
		for (i = last; i >= last-searchIncrement; i -= 1)
			{
			if i < 0
				{
	...
	List_ContextMenu(x, y)
		...
		if menufuncs.Member?(index)
			if true is (menufuncs[index])(:sel, source: this)
				.list.Repaint()
		}
