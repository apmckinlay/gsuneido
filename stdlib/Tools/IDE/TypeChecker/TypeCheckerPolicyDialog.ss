...
Controller
	...
	CallClass(policy)
		{
		return OkCancel([this, policy], .Title)
		}
	...
	layout(policy)
		...
		for name in policy.Members().Sort!()
			...
			vert.Add(
				[#Pair, [#Static, name],
					[#ChooseList, .levels, width: 6, mandatory:, :name]])
			}
		if policy.Member?(#confidence)
			vert.Add(
				[#Pair, [#Static, #confidence],
					[#Horz,
						[#ChooseList, .ops, width: 4, mandatory:, name: #confidenceOp],
						[#Skip, small:],
						[#Number, name: #confidencePct, width: 5, mask: "###"],
						[#Static, '%']]])
		return [#Record, vert]
		}
