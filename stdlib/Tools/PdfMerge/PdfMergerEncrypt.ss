...
class
	...
	EncryptObj(obj, reader, f, encrypt, cleanupFiles)
		...
		if not obj.head.Trim().Suffix?(#stream) or not obj.tail.Trim().Prefix?(#endstream)
			return
		obj.head $= '\n'
		...
		encrypted =
			PdfEncrypt.Encrypt(streamData.ToHex(), encrypt.encryptionKey).FromHex()
	...
	EncryptHead(obj, encrypt)
		...
		obj.head = obj.head.
			...
			Replace(`(?q)\)`, `__SU_RIGHT_PAREN__`).
			Replace("\(.*?\)",
				{
				s = it[1..-1].
					Replace(`__SU_LEFT_PAREN__`, `\(`).
