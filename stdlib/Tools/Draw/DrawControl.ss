...
Controller
	...
	readOnly: false
	extraValues: ()
	New(valueOb = false, .readOnly = false, .extraItems = #(), .width = false,
		.height = false, .noJustify? = false)
		{
	...
	Controls()
		...
		lay = [#Vert,
			...
			[#Horz,
				[#DrawPalette, .extraItems],
				canvas],
			[#EtchedLine, before: 0, after: 0]]

		if .noJustify? is false
			lay.Add([#RadioButtons, #Left, #Center, #Right horz:])
	...
	On_Print()
		...
		Params.On_Preview(justifiedFmt,
			previewWindow: GetFocus()) /*, default_orientation: 'Landscape'*/
		}
	...
	WrapItems(items)
		{
		return [#DrawItem, CanvasGroup(items)]
		}
	...
	ApplyJustify(fmt, drawOb = false)
		...
		switch drawOb.GetDefault(#justify, #Left)
			{
		case #Left:
			return [#Horz, fmt, #Hfill xstretch: 1]
		case #Right:
			return [#Horz, #Hfill, fmt xstretch: 1]
		case #Center:
			return [#Horz, #Hfill, fmt, #Hfill xstretch: 1]
			}
		}
	...
	Get()
		...
		return Object(items: .canvas.Get(),
			justify: .justify is false ? #Left : .justify.Get()).MergeNew(.extraValues)
		}
	...
	BuildItems(valueOb, skipSetupScale? = false)
		...
		valueOb.items.Each()
			...
			fn = Suneido.Compile(
				"function() { " $ it $ (skipSetupScale? ? "" : ".SetupScale()") $ " }")
