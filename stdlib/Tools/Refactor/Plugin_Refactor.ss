...
#(Contributions: (
		(UI, action, menu: Refactor,
			...
			target: function(@args)
				{
				Refactor_Rename_Variable(@args)
				}),
		(UI, action, menu: Refactor,
			name: "Rename Member...",
			target: function(@args)
				{
				Refactor_Rename_Member(@args)
				}),
		(UI, action, menu: Refactor,
			name: "Extract Table...",
			target: function(@args)
				{
				Refactor_Table_Class(@args)
				}),
		(UI, action, menu: Refactor,
			name: "Extract Method...", accel: "Ctrl+Alt+M",
			target: function(@args)
				{
				Refactor_Extract_Method(@args)
				}),
//	(UI, action, menu: Refactor,
//		name: "Extract Member...",
//		target: function (@args) { Refactor_Extract_Member(@args) })
		(UI, action, menu: Refactor,
			...
			target: function(@args)
				{
				Refactor_Extract_Variable(@args)
				}),
		(UI, action, menu: Refactor,
			name: "Inline Variable...", accel: "Ctrl+Alt+I",
			target: function(@args)
				{
				Refactor_Inline_Variable(@args)
				}),
//	(UI, action, menu: Refactor,
...
//		target: function (@args) { Refactor_Extract_Function(@args) })
		(UI, action, menu: Refactor,
			...
			target: function(@args)
				{
				Refactor_Convert_Function_To_Class(@args)
				}),
