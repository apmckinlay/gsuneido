...
SvcTests
	...
	Test_Rename()
		...
		.svcTableBk.Output([name: #Child2, parent: rec.num, text: "Child2 Text",
		...
		// deletion
		// SvcLibrary
		QueryDo("update " $ .svcTableLb.NameQuery("Rename2") $ " set lib_committed = " $
			Display(Date()))
	...
	assertRename(svcTable, oldName, newName, stagedDelete = false, restoreDelete = false)
		...
		if restoreDelete
			...
			Assert(newRec.lib_committed isDate:)
	...
	Test_MoveBook()
		...
		QueryDo("update " $ book $ " where num is " $ rec.num $ " set lib_committed = " $
			Display(Date()))
	...
	Test_MaxCommitted()
		...
		for table in [.MakeLibrary(), .MakeBook()]
			...
			svcTable.Output([name: #Rec1, lib_committed: date = Date().Plus(hours: 1)],
				committed:)
	...
	deletedRec(name, lib_before_text = "", lib_before_path = "", lib_committed = "")
		{
		return [:name, :lib_committed, :lib_before_text,
			:lib_before_path].RemoveIf({ it is "" })
		}
	}
