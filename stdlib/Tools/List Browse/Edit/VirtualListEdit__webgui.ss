...
class
	...
	editCell(col)
		...
		custom = customFields isnt false ? customFields.GetDefault(col, false) : false
		readonly = FieldProtected?(col, .rec, .editModel.ProtectField)
		.Editing = true
		row = .model.GetRecordRowNum(.rec) + .model.Offset
		.editor = new ListEditWindow(0, readonly, .model.ColModel.FindCol(.col), row,
			this,
	...
	allowEdit(rec, col)
		...
		if String?(freshRec)
			return freshRec
		.rec = freshRec
		allow? = .parent.Controller.Send(#VirtualList_AllowCellEdit, col, rec: freshRec,
			oldrec: rec)
	...
	CommitChange(grid, rec, col, data, valid?, unvalidated_val = "")
		...
		if model.CheckBoxColModel isnt false and
			0 is grid.Controller.Send(#VirtualList_AllowAutoSelectByAmount)
			{
