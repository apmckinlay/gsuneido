...
class
	...
	New(protectField, option, .historyFields, .ctrl = false, warningField = false)
		...
		.Global = ["Reporter...", "Reporter Forms...", "Summarize...",
			"CrossTable...", "Export..."]
	...
	noHistoryMenu?(c)
		{
		return c[2] is #History and
			(.historyFields is false or
	...
	CrossTable(ctrl)
		{
		ToolDialog(ctrl.Window.Hwnd,
			Crosstab(ctrl.GetQuery(), ctrl.GetTitle(), ctrl.GetExcludeSelectFields(),
	...
	Export(ctrl)
		{
		GlobalExportControl(ctrl.GetQuery(),
			excludeSelectFields: ctrl.GetExcludeSelectFields(),
			columns: ctrl.GetFields())
		}
	}
