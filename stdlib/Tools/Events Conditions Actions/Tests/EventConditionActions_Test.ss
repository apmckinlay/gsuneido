...
Test
		...
		Plugins().ForeachContribution(#ECA, #action)
			...
			try
				...
				if func.
					Params().
					Split(", ").
					Remove("logError = false)").
					Size() isnt requiredArgs.Size()
		...
		.mockCase(#EventConditionActions, #GetEventSourceFields,
			...
			when: [
				...
				[#getPluginInfo, ["Test Result2", #info_type],
					[#ECA, #info_type, name: "Test Result2", source: table]],
				[#extraFields, [[anyArgs:]], ""]])
		}
	...
	Test_execute()
		...
		actionInfo = [#ECA, #action, name: "Test Action",
	...
	Test_getActionRequired()
		...
		.mockCase(#EventConditionActions, #getActionRequired,
			args: #("Test Event", [requires: ("Test Result")],
				(testarg: testing)),
			expect: #(testing111),
			when: [[#getPluginInfo, ["Test Event", #event],
					[#ECA, #event, name: "Test Event",
						produce: Object("Test Result": function(testarg)
							{
							return testarg $ "111"
							})]]])
		...
		.mockCase(#EventConditionActions, #getActionRequired,
			...
			when: [[#getPluginInfo, ["Test Event", #event],
					[#ECA, #event, name: "Test Event",
						produce: Object("Test Result": #Prompt)]]])
		}
	...
	Test_source_satisfied?()
		...
		.mockCase(#EventConditionActions, #satisfied?,
			...
			when: [
				[#getPluginInfo, ["Test Result", #info_type],
					[#ECA, #info_type, name: "Test Result", source: table]],
				[#getDetail,
		...
		.mockCase(#EventConditionActions, #satisfied?,
			args: #("Test Event",
				...
				(testarg: [rec_field: "testing record"])), expect:,
			when: [
				[#getPluginInfo, ["Test Result", #info_type],
					[#ECA, #info_type, name: "Test Result", source: table]],
				[#getDetail,
	...
	Test_applyForeignData()
		...
		func(rec, expectedField)
		Assert(rec
			is: [mastertable_job: "a thing", othertable_num: #NotLinkedButInDB,
	...
	Test_populateEventActions()
		...
		testCl = EventConditionActions
			{
			EventConditionActions_satisfied?(eventName, conditions/*unused*/,
				details/*unused*/) { return eventName.Has?(#SATISFIED) }
			}
	...
	Teardown()
		...
		if not .actionsTableExisted?
			try
				Database("drop event_actions")
		if not .conditionsTableExisted?
			try
				Database("drop event_condition_actions")
		}
	}
