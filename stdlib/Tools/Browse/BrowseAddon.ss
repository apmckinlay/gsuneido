...
Addon
	...
	Print(default_title, query)
		...
		ReportFromQueryAndColumns(.getSortedQuery(query), cols.Delete(0), title,
			.Parent.Window.Hwnd)
		}
	...
	getSortedQuery(query)
		...
		sort = .GetSort()
		if sort is "" or
			not QueryColumns(QueryStripSort(query)).Has?(sort.RemovePrefix("reverse "))
			return query
	...
	Restore(recQuery, observer_ListRow)
		...
		if sel is #()
			return
		row = sel[0]
		ctrlRecord = .GetRow(row)

		if ctrlRecord.Member?(#Browse_RecordDirty) and
			ctrlRecord.Browse_NewRecord isnt true
			{
			if ctrlRecord.listrow_deleted is true
				{
				.AlertInfo("Context Restore",
					"The record is marked for " $ "deletion. Please use Undelete first.")
				return
				}
			record = Query1(recQuery(ctrlRecord))
			if record is false
				{
			...
			.attachmentsMgr.RestoreOneByKey(ctrlRecord)
			}
		.Refresh_BrowseData()
		}
	...
	History()
		...
		if sel is #()
			return
		row = sel[0]
		ViewHistory(.GetRow(row), .Parent.Window.Hwnd, .historyFields)
		}

	UpdateHistory(rec, newrecord?)
		{
		if .historyFields is false or newrecord? is true
			return
		UpdateHistory(rec, .historyFields)
		}

	// .Historyfields is the member name generated by Addon.setOptions automatically
	// But we don't want the lower case 'f' in the name
	getter_historyFields()
		{
		return .GetDefault(#Historyfields, false)
		}
	...
	LogColumnsSaveWarning(title)
		{
		warn = "WARNING: Browse Control is using query as " $ "user columns key: " $ title
	...
	alertNoOriginalRecord(linkField)
		...
		if linkField isnt false
			msg $=
				"Please use Current > Restore and " $ "re-do your changes if necessary."
	...
	standaloneMenu: ("Print...", "Reporter...", "Reporter Forms...", "Summarize...",
		"CrossTable...", "", "Save All", "Restore All", "", "Export...")
	readonlyStandaloneMenu: ("Print...", "Reporter...", "Reporter Forms...",
	...
	addEditingMenu(new_menu)
		...
		if selected isnt #()
			...
			if selected.Size() is 1
				...
				.addToMenu(new_menu, ["Edit Field"], 0)
				}
			}
		}
