...
PassthruController
	...
	New(query, columns = false, linkField = false, stickyFields = false,
		validField = false, protectField = false, .title = false, statusBar = false,
		.headerFields = #(), dataMember = #browse_data, .noShading = false,
		.noHeaderButtons = false, notifyLast = false, .columnsSaveName = "",
		.buttonBar = false, .headerSelectPrompt = false, .mandatoryFields = #(),
		stretch = false, .alwaysReadOnly = false, name = false, extra = false,
		hideContents = false, .noSaveOnDestroy = false, .loadRecordNotification = false,
		.addons = #(), limitSize? = false, .historyFields = false)
		{
	...
	buildCustomKey(name, query)
		...
		return String?(access_custom_key) ? access_custom_key $ " | " $ name : false
		}
	...
	createControls(statusBar, stretch, extra, limitSize?)
		...
		list = [stretch is true ? #ListStretch : #List, defWidth: false,
	...
	addStatusBarAndTitle(controls, extra, list, statusBar)
		{
		if .title isnt false
			controls.Add([#CenterTitle, .title])
		if extra isnt false
			{
			controls.Add(extra)
			controls.Add(#(Skip, 5))
	...
	SetQuery(query, columns = false, header_data = false, t = false, max_records = false,
		max_records_msg = "", exclude_custom_columns = false)
		{
		.list.FinishEdit()
		if query is ""
			return
			...
			tran.Query(query)
				...
				while false isnt x = q.Next()
					...
					if max_records isnt false and count >= max_records
						{
						status_msg = max_records_msg is "" and max_records isnt false
	...
	SetField(field, value, idx = false, invalidate = false)
		...
		if invalidate
			{
			.Send(#InvalidateFields, [.dataMember, .allDataMember])
	...
	Save(tran = false)
		...
		catch_block:
			...
			if .nextSaveRec isnt false and false isnt (i = .list.Get().Find(.nextSaveRec))
				.list.SetSelection(i)
			return false
			})
	...
	handleDeletedItems(data, tran, query, deletes, oldrecs)
		...
		if not valid
			.setBrowseStatusBar(
				"Another user has modified records, " $ "restore or discard changes",
				false)
		return valid
		}
	...
	existingRecordChanged?(record)
		...
		return ((record.Member?(#Browse_RecordDirty) and
				not record.Member?(#Browse_NewRecord)) or record.listrow_deleted is true)
		}
	...
	after_save_processing(data, deletes)
		...
		for record in data
			if record.Member?(#Browse_NewRecord) or record.Member?(#Browse_RecordDirty)
	...
	skipValid?(rec)
		...
		if rec.Member?(#Browse_NewRecord) and rec.Browse_RecordDirty isnt true
			return true
		// bypass old non-dirty records
		if not rec.Member?(#Browse_NewRecord) and not rec.Member?(#Browse_RecordDirty)
			return true
		return false
		}
	...
	List_AfterEditWindowCommit(col, row, data, valid?, valueChanged?)
		...
		for column in .list.GetColumns().Members()
			if ListCustomize.MandatoryAndEmpty?(record, .list.GetCol(column),
				.customFields, .protectField)
	...
	Refresh_BrowseData()
		...
		.Send(#InvalidateFields, [.dataMember])
		}
	...
	allowDelete?(record)
		{
		return .allowNextDelete or
			ProtectRuleAllowsDelete?(record, .protectField,
	...
	List_Deletions(deletions/*unused*/)
		{
		.Send(#SetField, .allDataMember, .list.Get())
		.Send(#InvalidateFields, [.allDataMember])
		}
	...
	ValidateRowControlData(rec, row)
		...
		for col in .list.GetColumns().Members()
			{
			field = .list.GetCol(col)
			if ListCustomize.MandatoryAndEmpty?(rec, field, .customFields,
					.protectField) or false is .controlValidData?(rec, field)
				.list.AddInvalidCell(col, row)
			}
		}
	...
	On_Context_Customize()
		{
		if false is result =
			ListCustomize.Customize(.query, .columns, .customKey, .linkField, this)
			return
	...
	handle_customize_result(dirty, custom_fields)
		...
		if Object?(dirty) and dirty.screen and custom_fields isnt false and
			custom_fields.NotEmpty?()
			{
	...
	refreshAndNotifyChange(data, row, member, col)
		...
		if record.Browse_RecordDirty isnt true and .query_columns.Has?(member) and
			(not Record?(record.Browse_OriginalRecord) or
				record.Browse_OriginalRecord[member] isnt record[member])
			{
			record.Browse_RecordDirty = true
