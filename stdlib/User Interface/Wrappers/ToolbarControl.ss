...
PassthruController
	...
	Controls: (Vert, xstretch: 1, ystretch: 0,
		(EtchedLine, before: 0, after: 0),
		(HorzEqualHeight, ystretch: 1))

	SetButtons(buttons)
		{
		if .horz.GetChildren().NotEmpty?()
			.horz.RemoveAll()
	...
	addtoolbarbutton(cmd)
		{
		command = .commands.GetDefault(cmd, [bitmap: cmd])
		accel = command.accel
		...
		.horz.Append([#EnhancedButton, command: cmd, :image, :tip,
			imagePadding: singleLetter? ? 0.05 : 0.15, /*=padding*/ mouseEffect:,
			ystretch: 0, name: cmd])
		}
