...
CanvasItem
	...
	Paint()
		{
		_report.AddEllipse(.x1, .y1, .x2 - .x1, .y2 - .y1, .GetThick(), .GetColor(),
			.GetLineColor())
		}
	BoundingRect()
		{
	...
	StringToSave()
		{
		'CanvasEllipse(x1: ' $ Display(.x1) $ ', y1: ' $ Display(.y1) $
			', x2: ' $ Display(.x2) $ ', y2: ' $ Display(.y2) $ ')'
		}
	ObToSave()
		{
		return Object('CanvasEllipse', .x1, .y1, .x2, .y2)
		}
	ForeachHandle(block)
		{
		r = .BoundingRect()
		...
		block(r.x1, y)
		block(r.x2, y)
		.handles = Object(Object(:x, y: r.y1), Object(:x, y: r.y2),
