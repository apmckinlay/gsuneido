...
CommandParent
	...
	view: false
	New(model, view, query = false, columns = false, .validField = false,
		readonly = false, .title = "", .stickyFields = #(), status = true,
		.linkField = false, protectField = false, .noShading = false,
		.noHeaderButtons = false, primary_accessobserver = false, .disableOptions = #(),
		.buttonBar = false, .extraButtons = #(), .columnsSaveName = false,
		.Commands = #(), .excludeFields = #(), .historyFieldsPrefix = false)
		{
	...
	makeControls(view, query, columns, status)
		...
		controls.Add([#VertSplit,
			[#List, :columns, defWidth: false,
				noShading: .noShading, noHeaderButtons: .noHeaderButtons, resetColumns:,
				alwaysHighlightSelected:],
			[#Scroll, [#Border, v, border: 5]]])
	...
	allowInsert?(force)
		{
		rec = .linkField isnt false ? .model.NewRecord(add: false) : []
	...
	NewRecord(output = false)
		...
		rec = .view.Get()
		if rec.Explorer_NewRecord isnt true or .view.Dirty?() or
			rec.Explorer_RecordUpdated is true
			rec = .model.NewRecord()
		rec.Explorer_NewRecord = true
		...
		.ignore_selection? = true
		if rec isnt .view.Get()
			// only add if we got a new record from the model
			.list.AddRow(rec)
		.ignore_selection? = false
	...
	SetWhere(where, join? = false) // called by Select
		...
		if .linkField isnt false
			.query =
				QueryAddWhere(.query, " where " $ .linkField $ " is " $ Display(.value))
	...
	SaveRecord()
		...
		keyExceptionResult = KeyException.TryCatch()
			...
			saveResult = x.Explorer_NewRecord is true ? .Output() : .Update()
			if saveResult
	...
	deleteUntouchedNewRec(x)
		...
		if x.Explorer_NewRecord is true
			...
			.ignore_selection? = true
			list_data = .list.Get()
			if false isnt (i = list_data.Find(x)) and list_data.Size() > 1
				.list.DeleteRows(i)
			.ignore_selection? = false
			}
		}
	...
	List_ContextMenu(x, y)
		...
		if .readonly
			return
		menu = [#New, #Delete, "", #Save, #Restore]
		if .linkField isnt false
			menu.Add("Restore All")
		else
			menu.Add("", "Reporter...", "Reporter Forms...", "Summarize...",
				"CrossTable...", "Export...")
