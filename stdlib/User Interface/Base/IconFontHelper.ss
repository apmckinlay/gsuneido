...
class
	{
	Fonts: (
		"suneido.ttf": (arrow_down, arrow_end, arrow_home, arrow_up, back, forward,
			...
			parent_folder, commentline, commentspan, gear, delete,
			...
			menu, lambda),
		"suneido2.ttf": (type, select, "rounded-rectangle", rectangle, line, image,
	...
	GetFontStyles(buildUrlFn)
		...
		styles = Object()
		for fontName in .Fonts.Members()
			styles.Add(`@font-face { ` $ `font-family: "` $
				fontName.RemoveSuffix(".ttf") $ `"; ` $ `src: url("` $
				buildUrlFn(fontName) $ `") format("truetype"); }`)
		return styles
		}
	}
