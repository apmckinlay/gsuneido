...
func TestTableOptimize2(t *testing.T) {
	...
	// ReqOrdered — match
	test("table", Require{use: ReqOrdered, cols: []string{"a"}}, 1, []string{"a"})

	// ReqOrdered — no match (b not a prefix of {a})
	assertImpossible("table", Require{use: ReqOrdered, cols: []string{"b"}}, 1)

	// ReqGrouped — match
	test("table", Require{use: ReqGrouped, cols: []string{"a"}}, 1, []string{"a"})

	// ReqGrouped — b not found in any index
	assertImpossible("table", Require{use: ReqGrouped, cols: []string{"b"}}, 1)

	// ReqLookup — {a} is a physical index
	test("table", Require{use: ReqLookup, cols: []string{"a"}}, 1, []string{"a"})

	// ReqLookup — {b} is not a physical index, no lookup-eligible index has {b}
	assertImpossible("table", Require{use: ReqLookup, cols: []string{"b"}}, 1)
	...
	test("supplier", Require{use: ReqOrdered, cols: []string{"supplier"}}, 1, []string{"supplier"})

	// ReqOrdered — match non-key index
	test("supplier", Require{use: ReqOrdered, cols: []string{"city", "supplier"}}, 1, []string{"city", "supplier"})

	// ReqOrdered — no match (name not in any index)
	assertImpossible("supplier", Require{use: ReqOrdered, cols: []string{"name"}}, 1)

	// ReqOrdered — city is a prefix of [city,supplier], so indexFor finds it
	test("supplier", Require{use: ReqOrdered, cols: []string{"city"}}, 1, []string{"city", "supplier"})

	// ReqGrouped — {city} is grouped by [city,supplier]
	test("supplier", Require{use: ReqGrouped, cols: []string{"city"}}, 1, []string{"city", "supplier"})

	// ReqGrouped — {name} not in any index
	assertImpossible("supplier", Require{use: ReqGrouped, cols: []string{"name"}}, 1)

	// ReqLookup — {supplier} is a physical index
	test("supplier", Require{use: ReqLookup, cols: []string{"supplier"}}, 1, []string{"supplier"})

	// ReqLookup — {city,supplier} is a physical index
	test("supplier", Require{use: ReqLookup, cols: []string{"city", "supplier"}}, 1, []string{"city", "supplier"})
	...
	assertImpossible("supplier", Require{use: ReqLookup, cols: []string{"city"}}, 1)
	...
	test("abc", Require{use: ReqGrouped, cols: []string{"a"}}, 1, []string{"a", "b"})

	// ReqGrouped — {b} is grouped by [b] (cheapest)
	test("abc", Require{use: ReqGrouped, cols: []string{"b"}}, 1, []string{"b"})

	// ReqGrouped — {c} is grouped by [c]
	test("abc", Require{use: ReqGrouped, cols: []string{"c"}}, 1, []string{"c"})

	// ReqLookup — {b} is a physical index (and a key)
	test("abc", Require{use: ReqLookup, cols: []string{"b"}}, 1, []string{"b"})

	// ReqLookup — {c} is a physical index (and a key)
	test("abc", Require{use: ReqLookup, cols: []string{"c"}}, 1, []string{"c"})

	// ReqLookup — {a} is not a physical index. [a,b] is not
	// lookup-eligible (key {b} is not a prefix, key {c} is not in index).
	assertImpossible("abc", Require{use: ReqLookup, cols: []string{"a"}}, 1)
	...
	test("comp", Require{use: ReqLookup, cols: []string{"b", "a"}}, 1,
		[]string{"a", "b", "c"})
	...
	for _, req := range []Require{
		reqUnordered,
		{use: ReqOrdered, cols: []string{"x"}},
		{use: ReqGrouped, cols: []string{"x"}},
		{use: ReqLookup, cols: []string{"x"}},
	} {
