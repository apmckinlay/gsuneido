...
func class_new(t *Thread, as *ArgSpec, this Value, args []Value) Value {
	return this.(*SuClass).New(t, as)
}

var _ = method(class_BaseQ, "(class)")

func class_BaseQ(t *Thread, this Value, args []Value) Value {
...
func class_MethodQ(t *Thread, this Value, args []Value) Value {
	m := ToStr(args[0])
...
func class_MethodClass(t *Thread, this Value, args []Value) Value {
	m := ToStr(args[0])
	return nilToFalse(this.(Findable).Finder(t, func(c Value, mb *MemBase) Value {
		if _, ok := c.(*SuClass); ok {
			if x, ok := mb.Data[m]; ok && x.Type() == types.Function {
