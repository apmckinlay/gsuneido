...
func ReadSchemaChain(st *stor.Stor, off uint64) SchemaChain {
	...
	return SchemaChain{
		SchemaHamt: ht.Freeze(),
		offs:       reverse(offs),
		ages:       ages,
	}
}

func (ht SchemaHamt) read(st *stor.Stor, off uint64, tomb map[string]struct{},
...
// It returns a new SchemaChain. It does not modify the original.
...
func (c *SchemaChain) WriteChain(store *stor.Stor) (uint64, SchemaChain) {
	no := len(c.offs)
	...
	filter := func(ts *Schema) bool { return ts.lastMod >= oldest }
	if merge == no {
		filter = func(ts *Schema) bool { return !ts.isTomb() }
	}
	prevOff := uint64(0)
	if no > 0 && merge < no {
