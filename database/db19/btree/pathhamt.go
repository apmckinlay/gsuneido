...
func (nd *nodePath) with(gen uint32, item *path, key uint64, hash uint32, shift int) *nodePath {
	...
	nu := &nodePath{generation: gen}
	if shift+bitsPerPathNode < 32 {
		oldval := &nd.vals[iv]
		oldkey := oldval.Key()
		nu = nu.with(gen, oldval, oldkey, PathHash(oldkey), shift+bitsPerPathNode)
		nu = nu.with(gen, item, key, hash, shift+bitsPerPathNode)
	} else {
...
func (nd *nodePath) without(gen uint32, key uint64, hash uint32, shift int) (*nodePath, bool) {
	...
	ip := bits.OnesCount32(nd.bmPtr & (bit - 1))
	nu, ok := nd.ptrs[ip].without(gen, key, hash, shift+bitsPerPathNode) // recurse
	if nu != nil {
		nd.ptrs[ip] = nu
	} else { // child emptied
		nd.bmPtr &^= bit
