...
Returns a [sequence](<../Basic Data Types/Sequence.md>) of the results of calling the block with the index and the value. Map2 is normally accessed as object/sequence.Map2

For example:

``` suneido
Primes().Take(4).Map2({|i,v| i $ "->" $ v })
    => #("0->2", "1->3", "2->5", "3->7")
```
